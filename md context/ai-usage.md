...
| 도구 | 활용 영역 |
|---|---|
| **Claude Code** | 컴포넌트 구현 · 릴리즈 파이프라인 · Slack 자동 전송 · Jira 데이터 분석 · 리팩토링 · 문서 작성 |
...
### 1. 릴리즈 파이프라인 자동화 (2026.04 ~ 2026.07)

Jira·GitLab·Slack을 MCP로 연동해, 디자인 시스템 2종의 릴리즈 전 과정을 단계별로 자동화했습니다.
...
- 변경분을 실시간으로 CHANGELOG에 반영하는 규칙 정리
- Figma 컴포넌트 → 코드 변환 전용 서브에이전트 등 반복 작업 자동화 체계 구성
- 이 체계로 `@cadian/ui`·`@cadian/ui-admin`의 릴리즈·문서·팀 공유를 단독 운영

### 2. Figma → 코드 → Slack 파이프라인
...
**활용 방식**
- Figma 컴포넌트의 Variant · Props · 토큰 정보를 입력하면 `TSX + SCSS + 문서 페이지` 초안 생성
...
| Skeleton | DCC-24 | Shape × Size × Tone, pulse 애니메이션, 데모 예제 17종 |
...
- 문서 사이트의 데모·API 표 반복 구조를 자동 생성하여 문서화 누락 방지

> 문서화는 Storybook이 아니라 **자체 문서 사이트**(Component Playground · API 표 · 패턴 문서)로 합니다.
...
Claude Code → Figma MCP → 스펙 자동 추출
         ↓
TSX Props · SCSS 토큰 · 문서 사이트 데모에 직접 반영
```
...
- 이 흐름을 **Figma → 코드 변환 전용 서브에이전트**로 정리해 반복 호출
...
| 활용 영역 | AI 역할 | 내 역할 | 근거 |
|---|---|---|---|
| 릴리즈 파이프라인 | 이슈 생성 → 커밋 → 머지 → npm 게시 → 배포 검증 → 팀 공유 자동화 | 파이프라인 설계 · 릴리즈 판단 | 디자인 시스템 2종 릴리즈 45회 이상 |
| 컴포넌트 구현 | Figma 스펙 → TSX·SCSS·문서 페이지 초안 | Figma 정합성 검토 · 수정 · 확정 | Slack #x-design 기록 |
...
| 포트폴리오 생성 | Jira 전수 검색 · 분류 · 요약 | 사실 확인 · 추가 지시 | Jira MCP 세션 |
